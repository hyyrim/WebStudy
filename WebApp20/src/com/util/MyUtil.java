...
public class MyUtil
{
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		//-- 현재 페이지 설정값(이 페이지를 기준으로 보여주는 숫자가 달라져야 하기 때문)
		...
		*/
		...
		// Prev (→ 이전으로)
		n = currentPage - numPerBlock;	// 현재페이지 - 10 → 13페이지에서 3페이지로 이동하는것
		if ( (totalPage>numPerBlock) && (currentPageSetup>0) )
		...
		{
			if (page == currentPage) // 현재페이지와 같다면 스타일적용하고 페이지이동 x (현재페이지이기 때문에)
