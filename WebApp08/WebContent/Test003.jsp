...
<%
	...
	// 데이터 수신 → Test002.jsp 로 부터 … (name, tel)
	...
	// 쿼리문 실행 → 적용된 행의 갯수 반환 (int)
	int result = 0;
	result = stmt.executeUpdate(sql);
	
	stmt.close();
	//conn.close();   --> Connection 객체 자체를 close한 것
	DBConn.close();	  // 내가 정의한 클래스 객체를 close 한 것
	
	// 반환된 행의 갯수가 1보다 작은 상황이면…
	// → 따라오세요~ 에러페이지~!!! 로 안내
	// 그 이외의 상황이면 …
